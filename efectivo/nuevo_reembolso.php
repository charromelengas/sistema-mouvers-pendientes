<?php
    ...
    {
       $pedido = $_POST['nuped'];
       $cantidad = $_POST['cantreemb'];
       $metodo = $_POST['vimet'];
       $motivo = $_POST['moti'];
       $administrador = $_SESSION['admin'];

       $sql_query = "INSERT INTO reembolso (ped_reemb, cant_reemb, metodo_reemb, motivo_reemb, id_usuario, id_sta) VALUES (?,?,?,?,?,2)";
        $sentencia_agregar = $conn->prepare($sql_query);
        $sentencia_agregar->execute(array($pedido,$cantidad,$metodo,$motivo,$administrador));
        $sentencia_agregar = null;
        $conn = null;
        echo '<script language="javascript">alert("Reembolso registrado con éxito!");</script>';
    }
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <body class="app sidebar-mini">
    ...
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-plus-circle"></i> Registrar reembolso</h1>
        </div>
        <ul class="app-breadcrumb breadcrumb">
          <li class="breadcrumb-item">Añadir reembolso</li>
        </ul>
      </div>
      <form action="nuevo_reembolso.php" method="POST">
        <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Número de pedido</label>
                      <input class="form-control" name ="nuped"  type="text" required>
           </div>
           <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Cantidad a reembolsar</label>
                      <input class="form-control" name ="cantreemb"  type="text" required>
           </div>
           <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Vía/Método</label>
                      <input class="form-control" name ="vimet"  type="text" required>
           </div>
           <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Motivo del reembolso</label>
                      <input class="form-control" name ="moti"  type="text">
           </div>
