<?php
    ...
    if($_POST)
    {
       $pedido = $_POST['numeroped'];
       $suc = $_POST['sucur'];
       $chofer = $_POST['conduc'];
       $client = $_POST['cliente'];
       $efeccancel = $_POST['subt'];
       $observa = $_POST['obser'];

       $sql_query = "INSERT INTO cancelaciones (num_pedi, sucursal, nom_rep, nom_clie, canti_cancel, observacion, id_usu, id_stat) VALUES (?,?,?,?,?,?,1,2)";
        $sentencia_agregar = $conn->prepare($sql_query);
        $sentencia_agregar->execute(array($pedido,$suc,$chofer,$client,$efeccancel,$observa));
...
<html lang="en">
  ...
  <body class="app sidebar-mini">
    ...
    <main class="app-content">
      <div class="app-title">
        <div>
          <h1><i class="fa fa-plus-circle"></i> Registrar cancelación</h1>
        </div>
        <ul class="app-breadcrumb breadcrumb">
          <li class="breadcrumb-item">Añadir cancelación</li>
        </ul>
      </div>
      <form action="cancelacion.php" method="POST">
        <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Número pedido</label>
                      <input class="form-control" name ="numeroped"  type="text" required>
           </div>
           <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Sucursal</label>
                      <input class="form-control" name ="sucur"  type="text" required>
           </div>
           <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Conductor</label>
                      <input class="form-control" name ="conduc"  type="text" required>
           </div>
           <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Cliente</label>
                      <input class="form-control" name ="cliente"  type="text">
           </div>
           <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Subtotal</label>
                      <input class="form-control" name ="subt"  type="text" required>
           </div>
           <div class="form-group">
                      <label class="col-form-label" for="inputDefault">Observaciones</label>
                      <input class="form-control" name ="obser"  type="text">
           </div>
